...
public class BackendServerManager : MonoBehaviour
{
    ...
    public void UpdateNickname(string nickname, Action<bool, string> func)
    {
        Enqueue(Backend.BMember.UpdateNickname, nickname, bro =>
        {
            ...
            if (!bro.IsSuccess())
            {
                ...
                {
                    func(false, PlayerPrefs.GetInt("LangIndex") == 0 ? BackendServerManager.GetInstance().langaugeSheet[16].kor : BackendServerManager.GetInstance().langaugeSheet[16].eng);
                }
    ...
    {
        var callback = Backend.Probability.GetProbabilitys("4044", isOne ? 1 : 11);
    ...
    public void GetLangaugeSheet()
    {
        ...
        Enqueue(Backend.Chart.GetChartContents, langaugeSheetCode, callback =>
        {
            if (callback.IsSuccess())
            {
                ...
                LoginUI.GetInstance().ChangeLobbyScene();
            }
        });
    }
    #endregion
